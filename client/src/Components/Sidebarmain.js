...
export default function Sidebarmain() {
 ...
 const components = [Page1, Page2, Page3, Page4];
  ...
  return (
    <Box sx={{ display: 'flex' }}>
      ...
      >
        ...
        <Divider />
        <List>
          {['Publish a job', 'View my jobs', 'Edit Jobs', 'Settings'].map((text, index) => (
            <ListItem key={text} disablePadding
            onClick={()=>{setController(index)}}
            style={{  backgroundColor: controller === index ? '#1976D2' : 'inherit',
              color: controller === index ? 'white' : 'inherit',
            }}
            >
              <ListItemButton>
                {/* <ListItemIcon>
...
{components.map((Component, index) =>
        controller === index && <Component key={index} />
      )}
      </Main>
    </Box>
  );
